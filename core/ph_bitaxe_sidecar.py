...
def ph_brain_loop():
    ...
    Uses 'Irrational-Offset' Scheduling (PHI) to minimize system resonance.
    """
    print("--- PH-BITAXE SIDECAR ACTIVE ---")
    if not init_bitaxe_bridge():
        print("Running in Simulation Mode (No Hardware found).")

    # Hijacking logic to PRIORITIZE USB workloads based on spectral insight
    ...
    print("Rationale: Phi is used as an irrational-offset scheme to minimize beat-frequency resonance with the OS scheduler.")

    # Launch standard Stratum, but injected with USB logic
    while True:
        # 1. Serial I/O - Polling responses
        if bitaxe_serial and bitaxe_serial.in_waiting:
            print(bitaxe_serial.readline())

        # 2. Time-Crystal (Irrational) Wait
        # Instead of sleeping 1.0s, we sleep a dynamic amount based on the Golden Ratio.
        # This acts as a jitter-reduction method to avoid harmonic collisions with polling loops.
        # This is one of many potential schemes (jitter, prime-steps) chosen for its period-doubling avoidance.
